...
import { tutorialSidebarNav } from "@/lib/tutorials/sidebar";
import { Menu, X } from "lucide-react";
import { useState } from "react";

function TutorialsSidebar({ onNavigate }: { onNavigate?: () => void }) {
  const pathname = usePathname();

  return (
    <nav className="space-y-6 text-sm">
      {tutorialSidebarNav.map((section) => (
        ...
        {mobileOpen && (
          <div className="fixed inset-0 top-14 z-40 lg:hidden">
            ...
            <div className="relative h-full w-72 overflow-y-auto border-r border-white/10 bg-black p-6">
              <TutorialsSidebar onNavigate={() => setMobileOpen(false)} />
            </div>
          </div>
        )}
